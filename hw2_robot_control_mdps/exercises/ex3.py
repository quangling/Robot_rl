...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray: 
    ...
    target_qpos = lower_limits + (action + 1) * (upper_limits - lower_limits) / 2 
    return target_qpos 
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray: 
    ...
    """ 
    # Transform end-effector position to base frame 
    ...
    # Transform end-effector rotation to base frame 
    # Convert rotation matrices to quaternions 
    ee_quat_w = rot_mat_to_quat(ee_rot_w) 
    base_quat_w = rot_mat_to_quat(base_rot_w) 

    # Compute the end-effector quaternion in the base frame 
    # To transform the end-effector quaternion from world frame to base frame, we can use the following formula: 
    # ee_quat_base = base_quat_conj * ee_quat_w 
    # where base_quat_conj is the conjugate of the base quaternion, and * denotes quaternion multiplication. 
    base_quat_conj = quat_conjugate(base_quat_w) 
    ee_quat_base = quat_mul(base_quat_conj, ee_quat_w) 
    ee_quat_base = quat_normalize(ee_quat_base) 

    # Concatenate all components to form the observation vector 
